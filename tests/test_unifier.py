#!/usr/bin/env python3
"""
Script de teste para o unificador de produtos
"""
...
def test_unifier():
    """Testa o unificador com dados de exemplo"""
    
    # Dados de exemplo baseados nos resultados fornecidos
    test_data = {
        "droga_raia": {
            "pharmacy": "Droga Raia",
            "products": [
        ...
        "sao_joao": {
            ...
            "products": [
                ...
                {
                    "brand": "Ibuprofeno 600mg 20 Comprimidos",
                    "description": "600mg 20 Comprimidos",
                    "discount_percentage": 0,
                    "has_discount": False,
    ...
    print("=== Teste do Unificador de Produtos ===\n")
    ...
    product2 = test_data['sao_joao']['products'][0]    # Prati da São João
    
    similarity = unifier.calculate_similarity(product1, product2)
    print(f"   Similaridade entre produtos Prati: {similarity:.3f}")
    
    # Produtos que não deveriam ser similares
    product3 = test_data['droga_raia']['products'][0]  # Buprovil
    product4 = test_data['sao_joao']['products'][1]    # Ibuprofeno genérico
    ...
    with open('test_unifier_results.json', 'w', encoding='utf-8') as f:
        json.dump(unified_results, f, indent=2, ensure_ascii=False)
    
    print(f"\n4. Resultados salvos em 'test_unifier_results.json'")
